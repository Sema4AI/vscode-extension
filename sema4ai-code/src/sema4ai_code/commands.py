...
SEMA4AI_CREATE_ROBOT = "sema4ai.createRobot"  # Create Task Package (Robot)
...
SEMA4AI_LIST_ROBOT_TEMPLATES_INTERNAL = "sema4ai.listRobotTemplates.internal"  # Provides a list with the available Task Package (Robot) templates
SEMA4AI_CREATE_ROBOT_INTERNAL = "sema4ai.createRobot.internal"  # Actually calls rcc to create the Task Package (Robot)
SEMA4AI_UPLOAD_ROBOT_TO_CLOUD = "sema4ai.uploadRobotToCloud"  # Upload Task Package (Robot) to Control Room
...
SEMA4AI_UPLOAD_TO_NEW_ROBOT_INTERNAL = "sema4ai.uploadToNewRobot.internal"  # Uploads a Task Package (Robot) as a new Task Package (Robot) in the Control Room
SEMA4AI_UPLOAD_TO_EXISTING_ROBOT_INTERNAL = "sema4ai.uploadToExistingRobot.internal"  # Uploads a Task Package (Robot) as an existing Task Package (Robot) in the Control Room
SEMA4AI_RUN_IN_RCC_INTERNAL = "sema4ai.runInRcc.internal"  # Runs a custom command in RCC
SEMA4AI_RUN_ROBOT_RCC = "sema4ai.runRobotRcc"  # Run Task Package (Robot)
SEMA4AI_RUN_ACTION_FROM_ACTION_PACKAGE = "sema4ai.runActionFromActionPackage"  # Run Action (from Action Package)
SEMA4AI_DEBUG_ROBOT_RCC = "sema4ai.debugRobotRcc"  # Debug Task Package (Robot)
...
SEMA4AI_RUN_ROBOCORPS_PYTHON_TASK = "sema4ai.runRobocorpsPythonTask"  # Run Robocorp's Python Task
SEMA4AI_DEBUG_ROBOCORPS_PYTHON_TASK = "sema4ai.debugRobocorpsPythonTask"  # Debug Robocorp's Python Task
...
SEMA4AI_COMPUTE_ROBOT_LAUNCH_FROM_ROBOCORP_CODE_LAUNCH = "sema4ai.computeRobotLaunchFromRobocorpCodeLaunch"  # Computes a Task Package (Robot) launch debug configuration based on the robocorp code launch debug configuration
...
SEMA4AI_REFRESH_ROBOTS_VIEW = "sema4ai.refreshRobotsView"  # Refresh Task Packages (Robots) view
SEMA4AI_REFRESH_ROBOT_CONTENT_VIEW = "sema4ai.refreshRobotContentView"  # Refresh Task Package (Robot) Content view
...
SEMA4AI_REFRESH_CLOUD_VIEW = "sema4ai.refreshCloudView"  # Refresh Robocorp view
...
SEMA4AI_OPEN_ROBOT_TREE_SELECTION = "sema4ai.openRobotTreeSelection"  # Configure Task Package (Robot) (robot.yaml)
...
SEMA4AI_CLOUD_UPLOAD_ROBOT_TREE_SELECTION = "sema4ai.cloudUploadRobotTreeSelection"  # Upload Task Package (Robot) to Control Room
...
SEMA4AI_SUBMIT_ISSUE = "sema4ai.submitIssue"  # Submit issue to Robocorp
...
SEMA4AI_CONFIGURATION_DIAGNOSTICS_INTERNAL = "sema4ai.configuration.diagnostics.internal"  # Task Package (Robot) Configuration Diagnostics (internal)
SEMA4AI_CONFIGURATION_DIAGNOSTICS = "sema4ai.configuration.diagnostics"  # Task Package (Robot) Configuration Diagnostics
SEMA4AI_RCC_TERMINAL_NEW = "sema4ai.rccTerminalNew"  # Terminal with Task Package (Robot) environment
SEMA4AI_LIST_WORK_ITEMS_INTERNAL = "sema4ai.listWorkItems.internal"  # Lists the work items available for a Task Package (Robot)
SEMA4AI_UPDATE_LAUNCH_ENV = "sema4ai.updateLaunchEnv"  # Updates the environment variables used for some launch (given a Task Package (Robot))
...
SEMA4AI_CLEAR_ENV_AND_RESTART = "sema4ai.clearEnvAndRestart"  # Clear Robocorp (RCC) environments and restart Sema4.ai Code
