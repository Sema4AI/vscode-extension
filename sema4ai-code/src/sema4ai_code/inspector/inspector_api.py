...
from sema4ai_ls_core.python_ls import PythonLanguageServer
from sema4ai_ls_core.core_log import get_logger
from sema4ai_ls_core.constants import IS_WIN

from sema4ai_code.inspector.web._web_inspector import (
    PickedLocatorTypedDict,
    WebInspector,
)

if TYPE_CHECKING:
    from sema4ai_code.inspector.windows.windows_inspector import WindowsInspector
    from sema4ai_code.inspector.image._image_inspector import ImageInspector
    from sema4ai_code.inspector.java.java_inspector import JavaInspector
...
class _WebInspectorThread(threading.Thread):
    ...
    def run(self) -> None:
        ...
        try:
            ...
            try:
                from sema4ai_code.playwright.robocorp_browser._caches import (
                    clear_all_callback,
                )
...
class _WindowsInspectorThread(threading.Thread):
    ...
    def run(self) -> None:
        ...
        from sema4ai_code.inspector.windows.windows_inspector import (
...
class _WindowsParseLocator(_WindowsBaseCommand):
    ...
    def __call__(
        ...
        from sema4ai_code.inspector.windows.robocorp_windows._errors import (
            InvalidLocatorError,
        )
                ...
                if isinstance(params, OrSearchParams):
                    ...
                    if not isinstance(params, SearchParams):
                        raise InvalidLocatorError(
                            ...
                            "locator.\nPlease report this as an error to sema4ai-code."
                            f"\nLocator: {locator}"
                        )
                    if params.empty():
                        raise InvalidLocatorError(
...
class _JavaParseLocator(_JavaBaseCommand):
    ...
    def __call__(self, java_inspector_thread: _JavaInspectorThread) -> ActionResultDict:
        ...
        from sema4ai_code.inspector.java.robocorp_java._locators import parse_locator
