...
export function registerViews(context: ExtensionContext) {
    ...
    context.subscriptions.push(
        onSelectedRobotChanged(async (robotEntry: RobotEntry | undefined) => {
            if (!robotEntry) {
                vscode.commands.executeCommand("setContext", "sema4ai-code:single-task-selected", false);
                vscode.commands.executeCommand("setContext", "sema4ai-code:single-robot-selected", false);
                return;
            }
            vscode.commands.executeCommand(
                "setContext",
                "sema4ai-code:single-task-selected",
                robotEntry.type == RobotEntryType.Task
            );
            vscode.commands.executeCommand("setContext", "sema4ai-code:single-robot-selected", true);
        })
    );
