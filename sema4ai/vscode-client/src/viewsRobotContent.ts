...
import { TREE_VIEW_SEMA4AI_PACKAGE_CONTENT_TREE } from "./robocorpViews";
import { FSEntry, NO_PACKAGE_FOUND_MSG, RobotEntry, treeViewIdToTreeView } from "./viewsCommon";
...
export async function getCurrRobotTreeContentDir(): Promise<FSEntry | undefined> {
    let robotContentTree = treeViewIdToTreeView.get(TREE_VIEW_SEMA4AI_PACKAGE_CONTENT_TREE);
    if (!robotContentTree) {
        return undefined;
    }
