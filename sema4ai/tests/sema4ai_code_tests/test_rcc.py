...
def test_numbered_dir(tmpdir):
    from sema4ai_code.rcc import make_numbered_in_temp
...
def holotree_manager(robocorp_home, rcc):
    ...
    vscode_path = Path(robocorp_home) / ".sema4ai_code"
...
def test_get_robot_yaml_environ(rcc: IRcc, datadir, holotree_manager):
    ...
    from sema4ai_code.holetree_manager import UnableToGetSpaceName
    ...
    for _ in range(2):
        ...
        with space_info.acquire_lock():
            assert space_info.conda_contents_match(
                rcc, robot1.conda_yaml_contents, str(robot1.conda_yaml)
            )
    ...
    with space_info.acquire_lock():
        assert space_info.conda_contents_match(
            rcc, robot2.conda_yaml_contents, str(robot2.conda_yaml)
        )
    ...
    with space_info.acquire_lock():
        assert space_info.conda_contents_match(
            rcc, robot3.conda_yaml_contents, str(robot3.conda_yaml)
        )
