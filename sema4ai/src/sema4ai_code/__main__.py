...
def main(
    ...
    if args.version:
        sys.stdout.write(sema4ai_code.__version__)
        sys.stdout.flush()
        return

    configure_logger(log_prefix, verbose, log_file)
    log = get_logger("sema4ai_code.__main__")
    log_args_and_python(log, original_args, sema4ai_code)

    try:
        from sema4ai_code.options import Options, Setup

        Setup.options = Options(args)

        from sema4ai_ls_core.python_ls import (
            binary_stdio,
    ...
    try:
        if sys.version_info[0] <= 2:
            raise AssertionError(
                "Python 3+ is required for Sema4.ai.\nCurrent executable: "
                + sys.executable
            )
        main()
