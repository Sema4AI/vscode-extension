...
class GalleryActions:
    ...
    def extract_package(self, package_name: str, target_dir: Path) -> ActionResult[str]:
        try:
            import json
            ...
            if use_dir.exists():
                ...
                return ActionResult.make_failure(
                    f"Unable to extract package. Directory {use_dir} already exists!"
                )
            ...
            try:
                response = http.get(package_metadata["zip"])
    ...
    def list_packages(self) -> ActionResult[dict[str, _PackageVersionMetadata]]:
        """
        ...
        ret: dict[str, _PackageVersionMetadata] = {}
    ...
    def _load_metadata(self) -> ActionResult[_PackageMetadataRoot]:
        ...
        try:
            # Try to get it fresh from the internet
            response = http.get(
                "https://sema4ai-gallery-metadata.s3.amazonaws.com/gallery_metadata.json"
            )
